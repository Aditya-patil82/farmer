...
def init_db():
    ...
    Handles read-only filesystems gracefully (e.g., on Vercel).
    """
    try:
        conn = get_db()
        cursor = conn.cursor()
        ...
        print(f"[WARN] Read-only file system or DB error: {e}")
        # Vercel provides a read-only filesystem, so we just ignore writes if they fail.
